...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$begin1_1, {
    ...
    date1 <- input$date1_1[1]
    date2 <- input$date1_1[2]
  ...
  observeEvent(input$begin2_1, {
    ...
    withProgress(session, min = 0, max = num9*num10, {
      ...
      df1 <- cbind(Individual = c(1:nrow(df1)),df1)
      tmpdf <- df1
      ...
      animate_plot2 <- function(){
        for (i in seq(1, num10)) 
        {
          ...
          {
            matrix[,j] <- 1+(matrix[,j]-min(matrix[,j]))/(max(matrix[,j])-min(matrix[,j]))*(9-1)
          }
          
          colnames(matrix) <- c("EW","TW","EZ","SZ")
          pl <- pairs(matrix,main=paste0("Relationships among variables during evalution","(iter",i,")"),
