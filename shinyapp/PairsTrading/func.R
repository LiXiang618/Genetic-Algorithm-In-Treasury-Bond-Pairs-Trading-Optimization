adRound <- function(x,digits = 0)
{
  tmp <- floor(x*10^(digits+1))
  tmp <- ifelse(tmp%%10>=5,ceiling(tmp/10),floor(tmp/10))
  tmp <- tmp/10^digits
  return(tmp)
}
...
PlotAssets <- function(effect.price, assets.id){
  main.string <- paste0("YTM: ", assets.id[1],"-", assets.id[2])
...
PlotAssetsDy <- function(effect.price,assets.id){
  
  date <- index(effect.price)
  
  ts1 <- xts(effect.price[,1], order.by = date)
  ts2 <- xts(effect.price[,2], order.by = date)
  
  names(ts1) <- assets.id[1]
  names(ts2) <- assets.id[2]
  
  dy1 <- cbind(ts1,ts2)
  
  dyg1 <- dygraph(dy1, main = paste0("YTM: ", assets.id[1],"-", assets.id[2]),group = "pairtrading") %>% 
    dyAxis("y", label = "YTM (%)") %>%
...
PlotSpreadDy <- function(effect.price,assets.id){
  date <- index(effect.price)
  spread <- as.numeric(effect.price[,1]-effect.price[,2])
  quantile <- (floor(rank(spread))-1)/length(spread)*100
  meanSpread <- mean(spread)
  sdSpread <- sd(spread)
  ts3 <- xts(spread, order.by = date)
  ts4 <- xts(quantile, order.by = date)
  names(ts3) <- "spread"
  names(ts4) <- "quantile"
  dy2 <- cbind(ts3,ts4)
  dyg2 <- dygraph(dy2, main = "Spread and Quantile",group = "pairtrading") %>% 
    dySeries('spread', color='green', strokePattern = 'solid') %>% 
    dyAxis("y", label = "Spread (%)",drawGrid = FALSE) %>%
    dySeries('quantile',axis = 'y2', color='#DCB5FF') %>%
    dyAxis('y2',label = 'Quantile (%)',axisLineColor = '#DCB5FF', axisLabelColor = '#DCB5FF',  drawGrid = FALSE) %>% 
    dyLimit(meanSpread, color = 'green',strokePattern = 'solid') %>% 
    dyLimit(meanSpread+sdSpread, color = 'blue',strokePattern = 'solid') %>% 
    dyLimit(meanSpread-sdSpread, color = 'blue',strokePattern = 'solid') %>% 
    dyLimit(meanSpread-2*sdSpread, color = 'red',strokePattern = 'solid') %>% 
    dyLimit(meanSpread+2*sdSpread, color = 'red',strokePattern = 'solid') %>% 
    dyLegend(show="follow",width = 120) %>%
    dyRangeSelector() %>%
    dyRoller(rollPeriod = 1)
  
  day1 <- which(quantile>=99 | quantile<=1)
  day2 <- which(quantile>=99 | quantile<=1)+1
  
  len <- length(day1)
  
  if(is.na(date[day2][len]))
    len <- length(day1)-1
  
  for(i in 1:len)
  {
    dyg2 <- dyg2 %>% dyShading(from = date[day1][i], to = date[day2][i], color = "yellow")
    dyg2 <- dyg2 %>% dyAnnotation(date[day1][i],text = as.character(quantile[day1][i]),attachAtBottom = TRUE,width = 40,height = 20)
  }
  dyg2
}
...
PlotBetaAndPremiumDy <- function(beta, premium){
  ...
  ts1 <- xts(beta, order.by = date)
  ts2 <- xts(premium, order.by = date)
  names(ts1) <- "beta"
  names(ts2) <- "alpha"
  dy <- cbind(ts1,ts2)
  ...
  dyg <- dygraph(dy, main = "beta & alpha",group = "pairtrading") %>% 
    dySeries('beta', color='green', strokePattern = 'solid') %>% 
    dyAxis("y", label = "beta") %>%
    dySeries('alpha',axis = 'y2', color='red', strokePattern = 'solid') %>%
    dyAxis('y2',label = 'alpha') %>%
    dyLegend(show="follow",width = 120) %>%
    dyRangeSelector() %>%
    dyRoller(rollPeriod = 1)
}
